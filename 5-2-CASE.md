...
`ORDER BY`の中で`CASE`を利用すると条件に基づいてソートを行うことができる。以下のクエリではorder_statusがpending(保留中) → shipped(発送済み) → delivered(配達完了) → cancelled(キャンセル済み)の順に並べている。
```
...
+----+---------------+--------------+--------+---------------------+
```
`UPDATE`で値を更新する際にも`CASE`を使用することができる。以下のクエリではamountが`NULL`のもの0にし、amountが200より小さいのものを1割引きにしている。
```
mysql> select * from orders;
+----+---------------+--------------+--------+---------------------+
| id | customer_name | order_status | amount | created_at          |
